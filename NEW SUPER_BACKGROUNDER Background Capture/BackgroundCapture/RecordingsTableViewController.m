...
-(void)deleteSelectedRows{
    ...
    {
        NSMutableIndexSet *delSet = [[NSMutableIndexSet alloc] init];
        for (NSIndexPath* idp in selectedRows) {
        
            
            //NSMutableArray *mar = self.videoFiles;
            NSString *path = @"Documents/merged";
            path = [path stringByAppendingPathComponent:[self.videoFiles objectAtIndex:idp.row]];
            path = [NSHomeDirectory() stringByAppendingPathComponent:path];
            [[NSFileManager defaultManager] removeItemAtPath:path error:nil];
            [delSet addIndex:idp.row];
        }
        
        //[self.videoFiles removeObjectAtIndex:idp.row];
        [self.videoFiles removeObjectsAtIndexes:delSet];
    ...
    {
        for (NSString *pt in self.videoFiles) {
            NSString *path = @"Documents/merged";
...
-(void)updateDeleteButton{
    NSArray *selectedRows = [self.tableView indexPathsForSelectedRows];
    NSString *deleteButtonTitle = [NSString stringWithFormat:@"Delete (%d)", selectedRows.count];
    
    if (selectedRows.count == self.videoFiles.count || selectedRows.count == 0)
    {
        deleteButtonTitle = @"Delete All";
    }
    self.navigationItem.rightBarButtonItem.title = deleteButtonTitle;
}
...
-(IBAction)editButtonPressed:(id)sender{
    ...
    if (self.tableView.isEditing){
        ...
        self.navigationItem.rightBarButtonItem.title = @"Delete All";
